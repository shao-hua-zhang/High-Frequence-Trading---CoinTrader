...
class DepthCache(object):
    ...
    def getLength(self):
        return max(len(self._bids), len(self._asks))
...
class DepthCacheManager(object):
    ...
    def __init__(self, client, symbol, callback=None, refresh_interval=_default_refresh):
        ...
        """
        # define the depth data to fetch
        self._depth = 0
        self._client = client
        self._symbol = symbol
    ...
    def _init_cache(self):
        ...
        for msg in self._depth_message_buffer:
            if self._depth == 0:
                ...
                self._process_partial_depth_message(msg, buffer=True)
    ...
    def _start_socket(self):
        ...
        if self._depth == 0:
            ...
            self._bm.start_depth_socket(self._symbol, self._depth_event, depth=str(self._depth))
    ...
    def _depth_event(self, msg):
        ...
        if self._last_update_id is None:
            ...
            if self._depth == 0:
                ...
                self._process_partial_depth_message(msg)

    def _process_partial_depth_message(self, msg, buffer=False):
        """
        Process a depth20, 10, 5 event message.
        :param msg:
        :param buffer:
        :return:
        """
        if buffer and msg['lastUpdateId'] <= self._last_update_id:
        ...
        self._last_update_id = msg['lastUpdateId']
        print("last_update_id", self._last_update_id)

        # after processing event see if we need to refresh the depth cache
        if self._depth_cache.getLength() > 50 or (self._refresh_interval and int(time.time()) > self._refresh_time):
            self._init_cache()
...
class DepthNData(object):
    ...
    def process_data(self, data):
        self._bids = [(float(bidInfo[0]), float(bidInfo[1])) for bidInfo in data["bids"]]
        self._asks = [(float(askInfo[0]), float(askInfo[1])) for askInfo in data["asks"]]
...
class DepthDataProcessor(object):
    def __init__(self, client, symbol, callback=None, depth=20):
        """Initialise the DepthCacheManager
        ...
        self._bm = None
        self._depth_cache = DepthNData(self._symbol)
    ...
    def _start_socket(self):
        ...
        self._bm.start_depth_socket(self._symbol, self._process_partial_depth_message, depth=str(self._depth))

        self._bm.start()

    def _process_partial_depth_message(self, msg):
        """
        Process a depth20, 10, 5 event message.
        :param msg:
        :param buffer:
        :return:
        """
        if msg['lastUpdateId'] <= self._last_update_id:
            # ignore any updates before the initial update id
            return

        # add any bid or ask values
        self._depth_cache.process_data(msg)
