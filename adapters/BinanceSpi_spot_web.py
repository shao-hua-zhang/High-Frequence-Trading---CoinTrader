...
from .binance.depthcache import DepthCache, DepthCacheManager, DepthDataProcessor
...
class BinanceSpi_spot_web(trader.traderInterface.adapterTemplate):
    ...
    def regiestTickers(self):
        ...
        for ticker in self.workingTickers:
            ...
            if binanceTicker not in subscribedSymbolList:
                ...
                iManager = DepthDataProcessor(client=self._client, symbol=binanceTicker, callback=self.processDepthData)
    ...
    def processOrderExcution(self, json_data):
        ...
        # check if the symbol is in this system
        if not (json_data["s"] in self.BinanceTickerToOkexTickerDict):
            return
        ID = int(json_data["c"])
