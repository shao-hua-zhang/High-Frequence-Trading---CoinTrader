...
import pandas as pd
# from trader.options import get_args
...
class SMMStrategy(SimpleStrategy):
    def __init__(self, path2Config, cache):
        ...
        self.buy_count_tmp, self.sell_count_tmp = 0, 0
        self.buy_count_tmp_id, self.sell_count_tmp_id = [], []
    ...
    def initStrategy(self):
        ...
        self.stopTradingDeltaQty = min(3 * self.tradeVol, 2)

        self.aux_buy_sell_pair = [None, None, 0] # bid ask qty
        self.aux_buy_queue, self.aux_sell_queue = [], []
        self.aux_is_first = True
        self.aux_index = 0
        ...
        # self.DEBUG = self.args.is_debug
        # self.stop_trading = self.args.stop_trading
        ...
        self.isFirstBtcBalance =  True
        self.initBtcBalance = 70
        ...
        while True:
            try:
                ...
                for order in open_orders:
                    if (self.last_buy_order and order.orderID != self.last_buy_order.orderID) and (self.last_sell_order and order.orderID != self.last_sell_order.orderID):
                        order_id_list.append( (order.orderID, order.price, order.orderType) )
                        if (time.time() - order.orderCreateTime) > 300:
                            print("[Warn] btcCancel Long Time Order")
                            self.trader.cancelOrder(order)

                print("+++++++++++++++btccancelitorder ok+++++++++++++++++++", order_id_list)
            except Exception as e:
                print("[Error] btcIn cancel_lt_order Error",e)
            time.sleep(20)
    ...
    def on_depth(self, cur_depth):
        ...
        api = cur_depth.API
        # print("testOnDepth ap1, bp1: ap1: {}, bp1： {}".format(cur_depth.AskPrice, cur_depth.BidPrice))
        ...
        if self.first_start:
            ...
            self.priceMove_level = 1
            self.period_pm1 = 20
            self.lastAskPrice = [self.last_depth.LastPrice] * self.period_pm1
            self.lastBidPrice = [self.last_depth.LastPrice] * self.period_pm1
    ...
    def do_signal(self):
        # time.sleep(1.1)
        # net_buy = round((self.buy_count + self.buy_count_tmp - self.sell_count_tmp - self.sell_count)/self.tradeVol, 2)
        # net_sell = round((self.sell_count + self.sell_count_tmp - self.buy_count_tmp - self.buy_count)/self.tradeVol, 2)
        net_buy = round((self.buy_count - self.sell_count)/self.tradeVol, 2)
        net_sell = round((self.sell_count - self.buy_count)/self.tradeVol, 2)
        askPrice = self.last_depth.AskPrice
        bidPrice = self.last_depth.BidPrice

        print("test ap1, bp1: ap1: {}, bp1： {}".format(askPrice, bidPrice))
        # print("test bids: {}, asks: {}".format(len(self.last_depth.Bids), len(self.last_depth.Asks)))

        btcBalance = self.trader.getCoinBalance("btc", self.api)
        if btcBalance:
            if self.isFirstBtcBalance:
                self.initBtcBalance = btcBalance
                self.isFirstBtcBalance = False
            if btcBalance > 76:
                # self.exposure_stopselling = True
                self.exposure_stopbuying = True
                print("btc too high, stop buying", btcBalance)
            elif btcBalance < 65:
                self.exposure_stopselling = True
                print("btc too low, stop selling", btcBalance)
            else:
                print("btc is ", btcBalance)
            netbuyBtc = round( (btcBalance - self.initBtcBalance)/self.tradeVol, 2)
            netsellBtc = round( (self.initBtcBalance - btcBalance)/self.tradeVol, 2)
        else:
            netbuyBtc, netsellBtc = 0, 0

        net_buy = max(net_buy, netbuyBtc)
        net_sell = max(net_sell, netsellBtc)
        # self.exposure_stopselling = True
        ...
        if len(open_orders_ls) >= 3:
            cnt = 0
            print("bbigger than 3 orders")
            for order in open_orders:
                if order.orderID not in open_orders_ls:
                    print("bandand orderstatus is ", order.orderStatus)
                    continue
                if cnt > 0:
                    break
                if self.last_buy_order and order.orderID == self.last_buy_order.orderID:
                    ...
                    print("band orderstatus is ", order.orderStatus)
                    continue
                else:
                    print("bCancel Long Time Order", order.orderID)
                    self.trader.cancelOrder(order)
                    cnt += 1
            return

        if self.priceMove_level == 1:
            print("btcskip, pricemovement too big")
            return
        ...
        if self.exposure_stopbuying:
            ...
            print("btcstopselling ", ret1)
            return
        elif net_buy > 0.5:
            ret1 = self.aheadSellOne(-net_buy)
            ret2 = self.safeBuyOne(net_buy)
            print("positive netbuy ", ret1, " ", ret2)
            return
        elif net_sell > 0.5:
            ret1 = self.aheadBuyOne(-net_sell)
            ret2 = self.safeSellOne(net_sell)
            print("positive netsell ", ret1, " ", ret2)
            return
        elif self.spread_level == 2:
            ret1 = self.aheadBuyOne(net_buy)
            ret2 = self.aheadSellOne(net_sell)
            print("btcsahead", ret1, " ", ret2)
            return
            ...
            print("btctoo volatile and activate skipping ", ret1, " ", ret2)
            return
        elif self.flat_level == 1:
            # ret1 = self.testSellOne(net_sell)
            ret1 = self.protectedSellOne(net_sell)
            # ret1 = self.aheadSellOne(net_sell)
            print("btcflatselling ", ret1)
            # print("btctemp skipping")
            return
        elif self.flat_level == 2:
            # ret1 = self.testBuyOne(net_buy)
            ret1 = self.protectedBuyOne(net_buy)
            # ret1 = self.aheadBuyOne(net_buy)
            print("btcflatbuying ", ret1)
            # print("btctemp skipping")
            return
            ...
            # ret1 = self.aheadBuyOne(net_buy)
            # ret2 = self.aheadSellOne(net_sell)
            print("btctestbs ", ret1, " ", ret2)
            return


        print("btcdo nothing !!!!!!!!")

    def compute_priceMove_level(self):
        self.lastAskPrice.pop(0)
        self.lastAskPrice.append(self.last_depth.AskPrice)
        self.lastBidPrice.pop(0)
        self.lastBidPrice.append(self.last_depth.BidPrice)

        deltaSellPrice = self.lastAskPrice[-1] - self.lastAskPrice[-5]
        deltaBuyPrice = self.lastBidPrice[-1] - self.lastBidPrice[-5]
        if self.priceMove_level == 0:
            if abs(deltaSellPrice) > 1.5:
                self.priceMove_level = 1
                return self.priceMove_level
            elif abs(deltaBuyPrice) > 1.5:
                self.priceMove_level = 1
                return self.priceMove_level
            else:
                self.priceMove_level = 0
                return self.priceMove_level
        elif self.priceMove_level == 1:
            if abs(deltaSellPrice) < 1 and abs(deltaBuyPrice) < 1:
                self.priceMove_level = 0
                return self.priceMove_level
            else:
                self.priceMove_level = 1
    ...
    def compute_flat2_level(self):
        ...
        if len(self.flat_stack) >= num_flat:
            last_flat_list = self.flat_stack[-5:-1]
            count = 0
    ...
    def get_spread_level(self):
        midPrice = (self.last_depth.AskPrice + self.last_depth.BidPrice) * 0.5
        # if self.last_buy_order and round(self.last_buy_order.price, 8) == round(self.last_depth.BidPrice, 8) and round(self.last_buy_order.amount - self.last_buy_order.filledAmount, 8) == round(self.last_depth.BidVolumes[0] , 8):
        #     bidPrice = self.last_depth.Bids[1]
        # else:
        #     bidPrice = self.last_depth.BidPrice
        # if self.last_sell_order and round(self.last_sell_order.price, 8) == round(self.last_depth.AskPrice, 8) and round(self.last_sell_order.amount - self.last_sell_order.filledAmount, 8) == round(self.last_depth.AskVolumes[0], 8):
        #     askPrice = self.last_depth.Asks[1]
        # else:
        #     askPrice = self.last_depth.AskPrice
        # self.spread = askPrice - bidPrice
        self.spread = self.last_depth.AskPrice - self.last_depth.BidPrice
        # if (self.last_depth.AskPrice - self.last_depth.BidPrice) / midPrice > 7e-4:
    ...
    def aheadBuyOne(self, net_buy):
        if net_buy <= 0 and self.spread_level >= 1:
            # buyPrice = self.last_depth.BidPrice + self.get_price_miniMove(self.ticker, "None", self.api) * max(-10 * net_buy, 1)
            if self.last_depth.BidVolumes[0] > 0.01:
                buyPrice = self.last_depth.BidPrice + self.get_price_miniMove(self.ticker, "None", self.api)
            else:
                buyPrice = self.last_depth.BidPrice
            if self.last_buy_order and round(self.last_buy_order.price, 2) == round(self.last_depth.BidPrice, 2) and self.last_depth.AskPrice - buyPrice < 0.8:
                buyPrice = self.last_depth.BidPrice
            elif self.last_buy_order and round(self.last_buy_order.price, 2) == round(self.last_depth.BidPrice, 2) and self.last_buy_order.amount - self.last_buy_order.filledAmount == self.last_depth.BidVolumes[0]:
                buyPrice = self.last_depth.Bids[1] + self.get_price_miniMove(self.ticker, "None", self.api)
            else:
                pass
            ret = self.adjust_buy_order(buyPrice, "aheadBuyOne"+str(self.last_depth.BidPrice), 1)
            return ret
        elif net_buy <= 0 and self.spread_level == 0:
            if self.last_depth.BidVolumes[0] > 0.01:
                buyPrice = self.last_depth.BidPrice + self.get_price_miniMove(self.ticker, "None", self.api)
            else:
                buyPrice = self.last_depth.BidPrice
            # min(spread * 0.3, self.get_price_miniMove(self.ticker, "None", self.api) * max(-10 * net_buy, 1))
            if self.last_buy_order and round(self.last_buy_order.price, 2) == round(self.last_depth.BidPrice, 2):
                buyPrice = self.last_depth.BidPrice
            elif self.last_buy_order and round(self.last_buy_order.price, 2) == round(self.last_depth.BidPrice, 2) and self.last_buy_order.amount - self.last_buy_order.filledAmount == self.last_depth.BidVolumes[0]:
                buyPrice = self.last_depth.Bids[1] + self.get_price_miniMove(self.ticker, "None", self.api)
            else:
                pass
            ret = self.adjust_buy_order(buyPrice, "aheadBuyOne0"+str(self.last_depth.BidPrice), 1)
            return ret
        else:
            return self.protectedBuyOne(net_buy)

    def aheadSellOne(self, net_sell):
        if net_sell <= 0 and self.spread_level >= 1:
            # sellPrice = self.last_depth.AskPrice - self.get_price_miniMove(self.ticker, "None", self.api) * max(-10 * net_sell, 1)
            if self.last_depth.AskVolumes[0] > 0.01:
                sellPrice = self.last_depth.AskPrice - self.get_price_miniMove(self.ticker, "None", self.api)
            else:
                sellPrice = self.last_depth.AskPrice
            if self.last_sell_order and round(self.last_sell_order.price, 2) == round(self.last_depth.AskPrice, 2) and sellPrice - self.last_depth.BidPrice < 0.8:
                sellPrice = self.last_depth.AskPrice
            elif self.last_sell_order and round(self.last_sell_order.price, 2) == round(self.last_depth.AskPrice, 2) and self.last_sell_order.amount - self.last_sell_order.filledAmount == self.last_depth.AskVolumes[0]:
                sellPrice = self.last_depth.Asks[1] - self.get_price_miniMove(self.ticker, "None", self.api)
            else:
                pass
            ret = self.adjust_sell_order(sellPrice, "aheadSellOne"+str(self.last_depth.AskPrice), 1)
            # ret = ret + " " + str(sellPrice)
            return ret
        elif net_sell <= 0 and self.spread_level == 0:
            if self.last_depth.AskVolumes[0] > 0.01:
                sellPrice = self.last_depth.AskPrice - self.get_price_miniMove(self.ticker, "None", self.api)
            else:
                sellPrice = self.last_depth.AskPrice
            # min(spread * 0.3, self.get_price_miniMove(self.ticker, "None", self.api) * max(-10 * net_sell, 1))
            if self.last_sell_order and round(self.last_sell_order.price, 2) == round(self.last_depth.AskPrice, 2):
                sellPrice = self.last_depth.AskPrice
            elif self.last_sell_order and round(self.last_sell_order.price, 2) == round(self.last_depth.AskPrice, 2) and self.last_sell_order.amount - self.last_sell_order.filledAmount == self.last_depth.AskVolumes[0]:
                sellPrice = self.last_depth.Asks[1] - self.get_price_miniMove(self.ticker, "None", self.api)
            else:
                pass
            ret = self.adjust_sell_order(sellPrice, "aheadSellOne0"+str(self.last_depth.AskPrice), 1)
            # ret = ret + " " + str(sellPrice)
            return ret
        else:
    ...
    def testBuyOne(self, net_buy):
        ...
        pos = 0 if net_buy <= 0 else 1
        buyPrice = self.last_depth.BidPrice - max(2000 * net_buy, 1) * self.get_price_miniMove(self.ticker, "None", self.api) * (1 * pos) * ( 2 + self.volatility )  #* 2
        ret = self.adjust_buy_order(buyPrice, "testBuyOne", 1)
        return ret

    def testSellOne(self, net_sell):
        spread = self.last_depth.AskPrice - self.last_depth.BidPrice
        pos = 0 if net_sell <= 0 else 1
        sellPrice = self.last_depth.AskPrice + max(2000 * net_sell, 1) * self.get_price_miniMove(self.ticker, "None", self.api) * (1 * pos) * ( 2 + self.volatility ) #* 2
    ...
    def safeBuyOne(self, net_buy):
        spread = self.last_depth.AskPrice - self.last_depth.BidPrice
        pos = 0 if net_buy <= 0 else 1
        buyPrice = self.last_depth.BidPrice - max(5 * net_buy, 1) * self.get_price_miniMove(self.ticker, "None", self.api) * (600 + 100 * self.volatility) #* 2
    ...
    def safeSellOne(self, net_sell):
        spread = self.last_depth.AskPrice - self.last_depth.BidPrice
        pos = 0 if net_sell <= 0 else 1
        sellPrice = self.last_depth.AskPrice + max(5 * net_sell, 1) * self.get_price_miniMove(self.ticker, "None", self.api) * (600 + 100 * self.volatility) #* 2
    ...
    def protectedBuyOne(self, net_buy):
        ...
        for i in range(8):
            if self.last_buy_order and round(self.last_buy_order.price, 8) == round(self.last_depth.Bids[i], 8):
                ...
                bufferVol.append(self.bufferVol1)
            else:
                baseVol.append(0)
                bufferVol.append(self.bufferVol2)
        ...
        elif self.last_depth.BidVolumes[4] > baseVol[4] + bufferVol[4]:
            buyPrice = self.last_depth.Bids[4]
        elif self.last_depth.BidVolumes[5] > baseVol[5] + bufferVol[5]:
            buyPrice = self.last_depth.Bids[5]
        elif self.last_depth.BidVolumes[6] > baseVol[6] + bufferVol[6]:
            buyPrice = self.last_depth.Bids[6]
        # elif self.last_depth.BidVolumes[7] > baseVol[7] + bufferVol[7]:
        #     buyPrice = self.last_depth.Bids[7]
        # elif self.last_depth.BidVolumes[8] > baseVol[8] + bufferVol[8]:
        #     buyPrice = self.last_depth.Bids[8]
        else:
            buyPrice = self.last_depth.Bids[7]

        mult = 2 + self.volatility
        pos = 0 if self.volatility + 1 <= 1 else 1
        buyPrice = buyPrice - max(100 * net_buy * 0, pos) * self.get_price_miniMove(self.ticker, "None", self.api) * mult
    ...
    def protectedSellOne(self, net_sell):
        ...
        elif self.last_depth.AskVolumes[4] > baseVol[4] + bufferVol[4]:
            sellPrice = self.last_depth.Asks[4]
        elif self.last_depth.AskVolumes[5] > baseVol[5] + bufferVol[5]:
            sellPrice = self.last_depth.Asks[5]
        elif self.last_depth.AskVolumes[6] > baseVol[6] + bufferVol[6]:
            sellPrice = self.last_depth.Asks[6]
        # elif self.last_depth.AskVolumes[7] > baseVol[7] + bufferVol[7]:
        #     sellPrice = self.last_depth.Asks[7]
        # elif self.last_depth.AskVolumes[8] > baseVol[8] + bufferVol[8]:
        #     sellPrice = self.last_depth.Asks[8]
        else:
            sellPrice = self.last_depth.Asks[7]

        mult = 2 + self.volatility
        pos = 0 if self.volatility + 1 <= 0 else 1
        sellPrice = sellPrice + max(100 * net_sell * 0, pos) * self.get_price_miniMove(self.ticker, "None", self.api) * mult
    ...
    def simple_management_on_rtn_order(self, order, type="filled"):
        ...
        if curCoinAQty - self.initCoinAQty > 0:
            smm_pnl = curCoinBQty -self.initCoinBQty + (curCoinAQty - self.initCoinAQty) * curMidPirce #* curBidPrice
        else:
            smm_pnl = curCoinBQty -self.initCoinBQty + (curCoinAQty - self.initCoinAQty) * curMidPirce #* curAskPrice
        ...
        if curCoinAQty - self.initCoinAQty >= self.stopTradingDeltaQty:
            self.exposure_stopbuying = True
            print(curCoinAQty - self.initCoinAQty, self.stopTradingDeltaQty)
            self.log_warn("stop buying due to too much {}".format(self.coinA))
        else:
            self.exposure_stopbuying = False
        if curCoinBQty - self.initCoinBQty >= self.stopTradingDeltaQty * self.last_depth.LastPrice:
            self.exposure_stopselling = True
            print(curCoinBQty - self.initCoinBQty, self.stopTradingDeltaQty * self.last_depth.LastPrice)
            self.log_warn("stop selling due to too much {}".format(self.coinB))
        else:
            self.exposure_stopselling = False


        if delta_pnl <= -0.0004 * self.tradeVol * self.last_depth.LastPrice:
            self.pnl_stoploss = False
        else:
            self.pnl_stoploss = False
        if delta_pnl >= 0.0008 * self.tradeVol * self.last_depth.LastPrice:
        ...
        if type=="filled":
            ...
            if curCoinAQty - self.initCoinAQty > 0:
                self.smm_pnl = curCoinBQty -self.initCoinBQty + (curCoinAQty - self.initCoinAQty) * curMidPirce # * curBidPrice
            else:
                self.smm_pnl = curCoinBQty -self.initCoinBQty + (curCoinAQty - self.initCoinAQty) * curMidPirce # * curAskPrice
            ...
            out = out + "OrderFinished:{tag}, {qty}@{price}\ncumpnl (in .00001): {cumpnl}, deltapnl: {deltapnl}, total amount: {total_amount}, profitability: {profitability}, netbuy: {net_buy}\nwin rate: {win_rate}, loss rate: {loss_rate}, mean win: {mean_win}, mean loss: {mean_loss}, win to loss: {win2loss},\nmean pnl: {mean_pnl}, median pnl: {median_pnl}, 25pnl: {pnl25}, 75pnl: {pnl75}, max pnl: {max_pnl}, min_pnl: {min_pnl}".format(
                    ...
                    net_buy=round(self.buy_count + self.buy_count_tmp - self.sell_count_tmp - self.sell_count, 2),
        ...
        if self.smm_pnl < np.clip(-4* math.ceil(self.total_amount / 10000), a_max=-10, a_min=-50):
            self.log_warn("stop trading due to cum loss > -5 usdt")
            self.exitStrategy()
    ...
    def adjust_buy_order(self, bp, type, qty_mult = 1):
        ...
        if self.stop_trading:
            return
        obp = bp
        if round(self.last_depth.BidPrice - bp, 2) < round(self.ask_bid_one_tol, 2) and round(bp, 2) < round(self.last_depth.BidPrice, 2):
        ...
        if self.last_buy_order == None:
            ...
            ret = type + " order@{}|{}".format(bp, qty*qty_mult)
        ...
        elif self.last_buy_order and round(self.last_buy_order.price, 2) == round(bp, 2):
            ...
            if self.last_buy_order.isCancel == True:
                ...
                # self.last_buy_order.isCancel = False
                self.last_buy_order = None
            else:
                iszero = self.trader.cancelOrder(self.last_buy_order)
                if iszero == None:
                    Abandoned_Order = self.last_buy_order
                    print("btc"+"!"*20+"{} {}@{}".format(self.iszero_buy_count, Abandoned_Order.amount, Abandoned_Order.price))
                    self.iszero_buy_count += 1
                    if self.iszero_buy_count >= 10:
                        self.iszero_buy_count = 0
                        self.last_buy_order = None
                # if iszero == 1:
                #     print("use buy count tmp")
                #     self.buy_count_tmp += self.last_buy_order.amount
                #     self.buy_count_tmp_id.append(self.last_buy_order.orderID)
    ...
    def adjust_sell_order(self, ap, type, qty_mult = 1):
        if self.stop_trading:
            return
        oap = ap
        if round(ap - self.last_depth.AskPrice, 2) < round(self.ask_bid_one_tol, 2) and round(ap, 2) > round(self.last_depth.AskPrice, 2):
        ...
        if self.last_sell_order == None:
            ...
            ret = type + " order@{}|{}".format(ap, qty*qty_mult)
            if qty >= self.get_volume_base(self.ticker, "None", self.api):
        ...
        elif self.last_sell_order and round(self.last_sell_order.price, 2) == round(ap, 2):
            ret = type + " pass@{}".format(ap)
        else:
            ret = type + " cancel order@{}".format(self.last_sell_order.price)
            if self.last_sell_order.isCancel == True:
                print("[WARN:] cancel order with isCancel True")
                # self.last_sell_order.isCancel = False
                self.last_sell_order = None
            else:
                iszero = self.trader.cancelOrder(self.last_sell_order)
                if iszero == None:
                    Abandoned_Order = self.last_sell_order
                    print("btc"+"!"*20+"{} {}@{}".format(self.iszero_sell_count ,Abandoned_Order.amount, Abandoned_Order.price))
                    self.iszero_sell_count += 1
                    if self.iszero_sell_count >= 10:
                        self.iszero_sell_count = 0
                        self.last_sell_order = None
                # if iszero == 1:
                #     print("use sell count tmp")
                #     self.sell_count_tmp += self.last_sell_order.amount
                #     self.sell_count_tmp_id.append(self.last_sell_order.orderID)
    ...
    def on_rtn_order(self, order):
        if order.orderStatus == trader.defines.orderCancelled:
            print("------>>>>>>>>>>>>>>>>>>>>>>>>>>>>C rtn order {}|{}".format(round(time.time() ,4), order.orderID))
            if order.orderID in self.buy_count_tmp_id:
                self.buy_count_tmp_id.remove(order.orderID)
                self.buy_count_tmp -= order.amount
            elif order.orderID in self.sell_count_tmp_id:
                self.sell_count_tmp_id.remove(order.orderID)
                self.sell_count_tmp -= order.amount
            else:
                pass
            ...
            print("------>>>>>>>>>>>>>>>>>>>>>>>>>>>>F rtn order {}|{}".format(round(time.time() ,4), order.orderID))
            if order.orderID in self.buy_count_tmp_id:
                self.buy_count_tmp_id.remove(order.orderID)
                self.buy_count_tmp -= order.amount
            elif order.orderID in self.sell_count_tmp_id:
