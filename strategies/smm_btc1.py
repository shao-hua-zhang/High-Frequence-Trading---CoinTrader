...
class SMMStrategy(SimpleStrategy):
    ...
    def initStrategy(self):
        ...
        self.stopTradingDeltaQty = min(3 * self.tradeVol, 2) # Param

        self.bufferVol1 = 0.3 # Param
        self.bufferVol2 = 0.3 # Param

        self.num_flat = 5 # Param
        self.catch_flat = 3 # Param

        self.initBtcBalance = 70 # Param
        self.btcBalanceHi = 75 # Param
        self.btcBalanceLo = 65 # Param

        self.stopTradingLoss = -50 # Param
        self.deltaTime = 1 # Param

        self.strpre = "btc1 " # Param
    ...
    def do_signal(self):
        ...
        if btcBalance:
            ...
            if btcBalance > self.btcBalanceHi:
                # self.exposure_stopselling = True
                self.exposure_stopbuying = True
                print(strpre+"btc too high, stop buying", btcBalance)
            elif btcBalance < self.btcBalanceLo:
                self.exposure_stopselling = True
                print(strpre+"btc too low, stop selling", btcBalance)
            else:
                print(strpre+"btc is ", btcBalance)
        ...
        if self.priceMove_level == 1:
            print(strpre+"skip, pricemovement too big")
            return
        ...
        if self.exposure_stopbuying:
            ...
            print(strpre+"positive netbuy ", ret1, " ", ret2)
            return
        elif net_sell > 0.5:
            ret1 = self.aheadBuyOne(-net_sell)
            ret2 = self.safeSellOne(net_sell)
            print(strpre+"positive netsell ", ret1, " ", ret2)
            return
    ...
    def simple_management_on_rtn_order(self, order, type="filled"):
        ...
        if self.smm_pnl < np.clip(-5* math.ceil(self.total_amount / 10000), a_max=-10, a_min=self.stopTradingLoss):
