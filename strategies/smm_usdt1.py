...
class SMMStrategy(SimpleStrategy):
    ...
    def initStrategy(self):
        ...
        self.stopTradingDeltaQty = min(3*self.tradeVol, 10000) # Param
        ...
        self.aux_buy_queue, self.aux_buy_queue = [], []
    ...
    def on_depth(self, cur_depth):
        ...
        if self.first_start:
            ...
            self.value = self.get_balance(self.coinA, "None", self.api) * midPrice
    ...
    def do_signal(self):
        ...
        if usdtBalance:
            ...
            if usdtBalance > self.usdtBalanceHi:
                ...
                print(strpre+"stopselling since usdt is ", usdtBalance)
            elif usdtBalance < self.usdtBalanceLo:
                self.exposure_stopbuying = True
                print(strpre+"stopbuying since usdt is ", usdtBalance)
            else:
                print(strpre+"usdt is ", usdtBalance)
        ...
        if len(open_orders_ls) >= 4:
            cnt = 0
            print(strpre+"bigger than 4 orders")
    ...
    def adjust_buy_order(self, bp, type, qty_mult = 1):
        ...
        if self.stop_trading:
            return
        obp = bp
        if round(self.last_depth.BidPrice - bp, 8) < round(self.ask_bid_one_tol, 8) and round(bp, 8) < round(self.last_depth.BidPrice, 8):
        ...
        if self.last_buy_order == None:
            ...
            if iszero == None:
                ...
                print("!"*20 + str(self.iszero_buy_count))
    ...
    def adjust_sell_order(self, ap, type, qty_mult = 1):
        if self.stop_trading:
            return
        oap = ap
        if round(ap - self.last_depth.AskPrice, 8) < round(self.ask_bid_one_tol, 8) and round(ap, 8) > round(self.last_depth.AskPrice, 8):
        ...
        if self.last_sell_order == None:
            ...
            if iszero == None:
                ...
                print("!"*20 + str(self.iszero_sell_count))
